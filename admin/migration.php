...
/* AJAX Dismiss */
add_action( 'wp_ajax_megamaxmenu_nag_dismiss', 'megamaxmenu_migration_nag_dismiss_callback' );
function megamaxmenu_migration_nag_dismiss_callback(){
	check_ajax_referer( 'megamaxmenu-nag-dismiss' , 'nag_nonce' );
	update_option( MEGAMAXMENU_PREFIX.'migration_status' , 'do-not-migrate' );
	echo 1;
	die();
}

add_action( 'admin_init' , 'megamaxmenu_migration_check' , 100 );
function megamaxmenu_migration_check(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'megamaxmenu-settings' ){
		if( isset( $_GET['do'] ) ){
			...
			if( $_GET['do'] == 'migration-check' || 
				...
				update_option( MEGAMAXMENU_PREFIX.'migration_status' , false );
			}
	...
	if( is_array( $old_ops ) ){
		...
		if( $migration_status == false ){
			...
			$notice = __( 'Looks like you\'ve updated to MegaMaxMenu 3.  Awesome!  Would you like to migrate your settings?' , 'megamaxmenu' );
			$notice.= ' <a class="button button-primary" href="'.admin_url('themes.php?page=megamaxmenu-settings&do=migration-check&megamaxmenu_nonce='.wp_create_nonce( 'megamaxmenu-control-panel-do' )).'">'.__( 'Begin Migration' ).'</a>';
			$notice.= ' <a class="button megamaxmenu-migrate-dismiss" href="'.admin_url('themes.php?page=megamaxmenu-settings&do=no-migrate&megamaxmenu_nonce='.wp_create_nonce( 'megamaxmenu-control-panel-do' )).'">'.__( 'No, thanks' , 'megamaxmenu' ).'</a>';
			...
			if( $add ){
				...
				add_action( 'admin_footer' , 'megamaxmenu_migrate_nag_js' );
			}
		}
		else if( $migration_status == 'in_progress' ){
			$notice = __( 'Migration did not complete successfully' , 'megamaxmenu' );
	...
	//Migration not necessary, update flag so we don't bother checking again
		...
		update_option( MEGAMAXMENU_PREFIX.'migration_status' , 'n/a' );
	}
}

function megamaxmenu_migrate_nag_js(){
	?>
	<!-- MegaMaxMenu Migrate Nag AJAX -->
	<script>
		jQuery( document ).ready( function( $ ){
			$( '.megamaxmenu-migrate-dismiss' ).on( 'click' , function( e ){
				e.preventDefault();
				...
				//AJAX Dismiss
				var data = {
					'action' : 'megamaxmenu_nag_dismiss',
					'nag_nonce'	: '<?php echo wp_create_nonce( 'megamaxmenu-nag-dismiss' ); ?>'
				};
	...
	<!-- end MegaMaxMenu Migrate Nag AJAX -->
	<?php
}

add_action( 'admin_init' , 'megamaxmenu_migrate_settings' , 100 );
function megamaxmenu_migrate_settings(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'megamaxmenu-settings' ){
		...
		if( isset( $_GET['do'] ) && $_GET['do'] == 'migrate' ){
			...
			update_option( MEGAMAXMENU_PREFIX.'migration_status' , 'in_progress' );
			
			//MENU SETTINGS
			megamaxmenu_migrate_menu_settings();
			
			//MENU ITEM SETTINGS
			megamaxmenu_migrate_item_settings();

			//set flag
			update_option( MEGAMAXMENU_PREFIX.'migration_status' , 'complete' );
...
function megamaxmenu_migrate_item_settings(){
	...
	if( isset( $_GET['migrate_menu_ids'] ) ){
		...
		$menu_ids = _MEGAMAXMENU()->sanitize_data($_GET['migrate_menu_ids'], 'arrayabsint');
		...
		foreach( $menu_ids as $menu_id ){
			...
			foreach( $items as $item ){
				...
				$old_key = '_megamax_options';
				$old_meta = get_post_meta( $item->ID , $old_key , true );

				//up( $old_meta );

				//Migrate options
				$new_meta = megamaxmenu_menu_item_setting_defaults(); // todo
				...
				if( isset( $old_meta['menu-item-sidebars'] ) && $old_meta['menu-item-sidebars'] ){
					...
					if( $sidebar_id == 'wpmega-sidebar' ){
						$sidebar_id = 'megamaxmenu-sidebar-1';
					}
					else $sidebar_id = str_replace( 'wpmega-sidebar-' , 'megamaxmenu-sidebar-' , $sidebar_id );
				...
				//Update options
				update_post_meta( $item->ID , MEGAMAXMENU_MENU_ITEM_META_KEY , $new_meta );
				...
				do_action( 'megamaxmenu_after_menu_item_save' );
			...
			foreach( $menus as $m ){
				if( $m->term_id == $menu_id ){
					$notice = __( 'Successfully migrated Menu Item Settings for Menu: ' , 'megamaxmenu' ) . $m->name;
...
function megamaxmenu_migrate_menu_settings(){
	...
	$main_ops = megamaxmenu_get_instance_options( 'main' );
	$gen_ops = get_option( MEGAMAXMENU_PREFIX.'general' );
	...
	//This will merge your MegaMaxMenu 2 settings into MegaMaxMenu3, giving precedence to MegaMaxMenu 2 settings


	//Get NEW settings
	//Keep in mind Extensions - Sticky, Icons || [Not:Flat, Conditionals]
	//$new_fields = megamaxmenu_get_settings_fields();
	//up( $new_fields  , 2 );

	//echo 'main:';
	$config_id = 'main';
	$instance_ops = megamaxmenu_get_instance_options( $config_id );
	//up( $main_ops );

	//echo 'gen:';
	$gen_ops = megamaxmenu_get_instance_options( 'general' ); //get_option( MEGAMAXMENU_PREFIX.'general' );
	//up( $gen_ops );
	...
	$map_instance = array(	//NEW => OLD
		...
		//STICKY
		'sticky_offset'				=> 'megamaxsticky-top-spacing',
		'sticky_full_width'			=> 'megamaxsticky-expand-menu-bar',
		'sticky_clearfix'			=> 'megamaxsticky-clear',
		'sticky_center_inner_width'	=> 'megamaxsticky-center-inner',
		//sticky_background_color'	=> 'megamaxsticky-background-color', //Style Gen
		'sticky_mobile'				=> 'megamaxsticky-mobile',
		'sticky_permanent'			=> 'megamaxsticky-permanent',
		'sticky_apply_to'			=> 'megamaxsticky-apply-to',
	);
	...
	if( /*true || */ $old_ops['wpmega-style'] == 'inline' ){
		...
		$style_map_colors = array(
			...
			//Sticky
			'sticky_background_color'					=> 'megamaxsticky-background-color',
	...
	update_option( MEGAMAXMENU_PREFIX.$config_id , $instance_ops );
	...
	$map_general = array(
		...
		'load_megamaxmenu_css'			=> 'include-basic-css',
		...
		'megamaxmenu_theme_location'	=> 'wpmega-easyintegrate',

		//Sticky
		'sticky_toolbar_footer' 	=> 'megamaxsticky-wpadmin-bottom',
		'sticky_disable_css'		=> 'megamaxsticky-disable-css',
	);
	...
	update_option( MEGAMAXMENU_PREFIX.'general' , $gen_ops );
	
	//Regenerate Style Settings
	//megamaxmenu_generate_custom_styles();
	delete_transient( MEGAMAXMENU_GENERATED_STYLE_TRANSIENT );

	$notice = __( 'Successfully migrated Control Panel Menu Settings' , 'megamaxmenu' );
	add_settings_error( 'menu-migration' , 'migration-notice-menu-complete' , $notice , 'updated' );


}
//add_action( 'wp_head' , 'megamaxmenu_migrate_menu_settings' );
//megamaxmenu_migrate_menu_settings();