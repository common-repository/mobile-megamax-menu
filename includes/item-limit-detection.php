...
add_action( 'admin_notices',  'megamaxmenu_item_limit_notice' );

function megamaxmenu_count_menu_post_vars() {
	...
	if( isset( $_POST['save_menu'] ) ){
		...
		update_option( 'megamaxmenu-post-var-count' , $count );
	}
	else{
		$count = get_option( 'megamaxmenu-post-var-count' , 0 );
	}

	return $count;
}

function megamaxmenu_item_limit_notice(){

	$screen = get_current_screen();
	if( $screen->id != 'nav-menus' ) return;

	$var_count = megamaxmenu_count_menu_post_vars();
	...
	if( $r['suhosin_post_maxvars'] != '' ||
		...
		if( ( $r['suhosin_post_maxvars'] != '' && $r['suhosin_post_maxvars'] != 0 && $r['suhosin_post_maxvars'] < 1000 ) ||
			...
			$message[] = __( "Your server is running Suhosin, and your current maxvars settings may limit the number of menu items you can save." , 'megamaxmenu' );
		}
		...
		foreach( $r as $key => $val ){
			if( $val > 0 ){
				if( $val - $var_count < 150 ){
					$message[] = __( "You are approaching the post variable limit imposed by your server configuration.  Exceeding this limit may automatically delete menu items when you save.  Please increase your <strong>$key</strong> directive in php.ini." , 'megamaxmenu' );
				}
			}
		}
		...
		?>
		<div class="megamaxmenu-admin-notice megamaxmenu-admin-notice-warning megamaxmenu-admin-notice-menu-item-limit">
			<div class="megamaxmenu-admin-notice-icon"><i class="fas fa-exclamation-triangle"></i></div>
			<ul>
			...
			<?php endif; ?>
				...
				<tr>
					<td><?php _e( 'Last saved Item Variable Count' , 'megamaxmenu' ); ?></td>
				...
				<?php
					...
					<tr>
						<td><strong><?php _e( 'Estimated remaining menu items' , 'megamaxmenu' ); ?></strong></td>
