...
add_action( 'plugins_loaded', 'megamaxmenu_elementor_init' );

function megamaxmenu_elementor_init(){
  ...
  add_action( 'elementor/widgets/widgets_registered', 'megamaxmenu_elementor_init_widgets' );
	//add_action( 'elementor/controls/controls_registered', 'init_controls' );


}

function megamaxmenu_elementor_init_widgets() {
  // Include Widget files
	require_once( __DIR__ . '/widgets/nav-widget.php' );

	// Register widget
	\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new \Elementor_MegaMaxMenu_Nav_Widget() );
}
