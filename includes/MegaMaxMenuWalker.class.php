 ...
 * @uses Walker
 */
class MegaMaxMenuWalker extends Walker_Nav_Menu {
	...
	function __construct(){
		$this->setting_defaults = megamaxmenu_menu_item_setting_defaults();
...
// }
		...
		if( isset( $children_elements[ $element->$id_field ] ) && count( $children_elements[ $element->$id_field ] ) > 0 ){
			$has_row = false;
			$has_nonrow = false;
//if( $element->$id_field == 65 ) megamaxp( $children_elements[ $element->$id_field ] , 3 );

			foreach( $children_elements[ $element->$id_field ] as $child_el ){

				//If we find a [Row] child, flag
				if( $child_el->type_label == '[MegaMaxMenu Row]' ){
					$has_row = true;
				}
				...
				// else if( $child_el->object == 'megamaxmenu-custom' && $child_el->type_label == '[MegaMaxMenu Menu Segment]' ){
				// 	foreach( $children_elements[$child_el->ID] as $grandchild_el ){
				// 		if( $grandchild_el->type_label == '[MegaMaxMenu Row]' ){
				// 			$has_row = true;
				// 		}
				// 		else{
				// 			$has_nonrow = true;
					...
					foreach( $my_children as $k => $child_el ){
						...
						//If this isn't a row, wrap it in a row
							...
							if( !$current_row_id ){
								...
								$current_row = new MegaMaxMenu_dummy_item( $current_row_id , 'row' , 'Dummy Row' , $element->$id_field );
		...
		// Setup MegaMaxMenuItem object
		$umitem_obect_class = apply_filters( 'megamaxmenu_item_object_class' , 'MegaMaxMenuItemDefault' , $element , $id , $this->auto_child );
		...
		if( $display_on ){
			$display_on = apply_filters( 'megamaxmenu_display_item' , true , $this , $element , $max_depth, $depth, $args , $umitem );
		}
		...
		if( $umitem->getSetting( 'disable_submenu_on_mobile' ) == 'on' ){
			if( megamaxmenu_is_mobile( 'disable_submenu_on_mobile' ) ){
				$this->clear_children( $children_elements , $id );
