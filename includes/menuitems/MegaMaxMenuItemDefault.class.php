 ...
 * A regular menu item
 */
class MegaMaxMenuItemDefault extends MegaMaxMenuItem{
	...
	function get_new_column( $output ){
		...
		if( $this->depth >= 2 ){
			if( $this->getSetting( 'new_column' ) == 'on' ){			//New Columns
				...
				$output.= '</ul></li>';
				$output.= '<li class="megamaxmenu-item megamaxmenu-column megamaxmenu-column-'.$cols.' megamaxmenu-item-header megamaxmenu-newcol">'.
							'<span class="megamaxmenu-target">&nbsp;</span><ul class="megamaxmenu-submenu megamaxmenu-submenu-type-stack">';
			}
		}
	...
	function alter( &$children ){
		...
		if( $autocolumns && 													//Value is set
			...
			foreach( $column_map as $_col => $column_count ){
				...
				$new_children[$this->ID][] = new MegaMaxMenu_dummy_item( 
							$column_id , 
							'column' , 
							'Auto Column' , 
							$this->ID,
