 ...
 * Parent Class
 */
abstract class MegaMaxMenuItem{
	...
	*/
	...
	function __construct( &$output , &$item , $depth = 0, &$args = array() , $id = 0 , &$walker , $has_children = false ){
		...
		if( isset( $this->item->object ) && ( $this->item->object == 'megamaxmenu-custom' ) ){
		...
		//define( 'MEGAMAXMENU_ALLOW_NAV_MENU_ITEM_ARGS_FILTER' , true );
		//in the wp-config.php
		if( MEGAMAXMENU_ALLOW_NAV_MENU_ITEM_ARGS_FILTER ) $this->args = apply_filters( 'nav_menu_item_args' , $this->args , $this->item );
	...
	function get_transient_key( $prefix ){
		return 'megamaxtk_'.$prefix.$this->walker->unique_path_key( $this->get_id() );
	}
	...
	function display_on(){
		...
		if( $this->getSetting( 'disable_on_mobile' ) == 'on' ){
			if( megamaxmenu_is_mobile( 'disable_on_mobile' ) ){
				return false;
			}
		}

		if( $this->getSetting( 'disable_on_desktop' ) == 'on' ){
			if( !megamaxmenu_is_mobile( 'disable_on_desktop' ) ){
				return false;
			}
		}
	...
	function get_settings(){
		...
		$settings = get_post_meta( $this->item->ID, MEGAMAXMENU_MENU_ITEM_META_KEY , true );
		...
		$settings = apply_filters( 'megamaxmenu_item_settings' , $settings , $this->ID );
	...
	function get_menu_op( $op ){
		...
		//$instance = 'main';		//TODO
		//$instance = $this->args[0]->megamax_instance;

		$instance = $this->get_config_id();
		return megamaxmenu_op( $op , $instance );
	}

	function get_config_id(){
		return $this->args->megamax_instance;
	}
	...
	function get_submenu_type( $submenu_type = false ){
		...
		if( $submenu_type == 'auto' ){
			...
			//$classes[] = 'megamaxmenu-submenu-type-auto';
...
//echo ' -- ' .$this->item->title . ' : ' . $parent_submenu . '<br/>';
	...
	function get_submenu_id(){
		return 'megamaxmenu-submenu-'.$this->getID();
	}

	function initialize_submenu(){

		//Standard WordPress Classes
		$this->submenu_classes[] = 'megamaxmenu-submenu';
		$this->submenu_classes[] = 'megamaxmenu-submenu-id-' . $this->item->ID;


		//Submenu Type
		$submenu_type = $this->getSetting( 'submenu_type' );
		if( $submenu_type == 'auto' ){
			$this->submenu_classes[] = 'megamaxmenu-submenu-type-auto';
			$submenu_type = $this->get_submenu_type( $submenu_type );
		}

		$this->settings['submenu_type_calc'] = $submenu_type;

		$this->submenu_classes[] = 'megamaxmenu-submenu-type-'.$submenu_type;

		if( in_array( $submenu_type , array( 'mega' , 'flyout' ) ) ){
			$this->drop_sub = true;
			$this->submenu_classes[] = 'megamaxmenu-submenu-drop';
		}
	...
	function get_submenu_wrap_start(){
		...
		if( $submenu_type == 'mega' ){
			$classes[] = 'megamaxmenu-submenu-align-' . $this->getSetting( 'submenu_position' );
		}
		else if( $submenu_type == 'flyout' ){
			$classes[] = 'megamaxmenu-submenu-align-' . $this->getSetting( 'flyout_submenu_position' );
		}
		...
		if( $submenu_content_align && $submenu_content_align != 'default' ){
			$classes[] = 'megamaxmenu-submenu-content-align-' . $submenu_content_align;
		}
		...
		if( $this->getSetting( 'submenu_column_autoclear' ) == 'on' && $submenu_col_default != 'auto' && $submenu_col_default != 'natural' ){
			$classes[] = 'megamaxmenu-autoclear';
		}

		//Padding
		if( $this->getSetting( 'submenu_padded' ) == 'on' ){
			$classes[] = 'megamaxmenu-submenu-padded';
		}

		//Background Image
		if( $this->getSetting( 'submenu_background_image' ) ){	//Not 'on', image URL
			$classes[] = 'megamaxmenu-submenu-bkg-img';
		}

		//Submenu Grid
		if( $this->getSetting( 'submenu_grid' ) == 'on' ){
			$classes[] = 'megamaxmenu-submenu-grid';
		}


		//Indent
		if( $this->getSetting( 'submenu_indent' ) == 'on' ){
			$classes[] = 'megamaxmenu-submenu-indent';
		}

		//Retractors
		$retractor_top = $this->closable_sub && megamaxmenu_display_retractors() && ( megamaxmenu_op( 'display_retractor_top' , $this->args->megamax_instance ) == 'on' );
		if( $retractor_top ) $classes[] = 'megamaxmenu-submenu-retractor-top';

		//Close Button
		$close_button = $this->closable_sub && ( megamaxmenu_op( 'display_submenu_close_button' , $this->args->megamax_instance ) == 'on' );
		if( $close_button ){
			if( $retractor_top ) $classes[] = 'megamaxmenu-submenu-retractor-top-2';
			else 				 $classes[] = 'megamaxmenu-submenu-retractor-top';
		}
		...
		*/
		...
		$aria = '';
		if( megamaxmenu_op( 'aria_hidden' , 'general' ) == 'on' ){
		...
		$id = '';
		if( megamaxmenu_op( 'aria_controls' , 'general' ) == 'on' ){
		...
		if( $retractor_top ){
			...
			$retractor_label = megamaxmenu_op( 'retractor_label' , $this->args->megamax_instance );
			if( !$retractor_label )	$retractor_label = __( 'Close' , 'megamaxmenu' );

			$item_output.= '<'.$retractor_tag.' class="megamaxmenu-retractor megamaxmenu-retractor-mobile"><'.$retractor_icon_tag.' class="fas fa-times"></'.$retractor_icon_tag.'> '.$retractor_label.'</'.$retractor_tag.'>';
		}

		//Close button
		if( $close_button ){
			$retractor_tag = $this->submenu_tag == 'ul' ? 'li' : 'div';
			$item_output.= '<'.$retractor_tag.' class="megamaxmenu-retractor megamaxmenu-retractor-desktop"><'.$retractor_icon_tag.' class="fas fa-times"></'.$retractor_icon_tag.'></'.$retractor_tag.'>';
		}
	...
	function get_submenu_wrap_end(){
		...
		if( $footer_content ){
			...
			$html.= '<'.$fc_tag.' class="megamaxmenu-submenu-footer megamaxmenu-submenu-footer-id-'.$this->ID.'">'.$footer_content.'</'.$fc_tag.'>';
		}

//'tab-content-panel'

		//Retractor Bottom
		if( $this->closable_sub && megamaxmenu_display_retractors() && ( megamaxmenu_op( 'display_retractor_bottom' , $this->args->megamax_instance ) == 'on' ) ){
			$retractor_tag = $this->submenu_tag == 'ul' ? 'li' : 'div';
			$retractor_icon_tag = $this->get_menu_op( 'icon_tag' );
			if( !$retractor_icon_tag ) $retractor_icon_tag = 'i';
			...
			$html.= '<'.$retractor_tag.' class="megamaxmenu-retractor megamaxmenu-retractor-mobile"><'.$retractor_icon_tag.' class="fas fa-times"></'.$retractor_icon_tag.'> '.$retractor_label.'</'.$retractor_tag.'>';
		}
	...
	function add_class_item_defaults(){
		...
		if( is_array( $this->item->classes ) ){
			...
			if(
				...
				( megamaxmenu_op( 'scrollto_disable_current', 'general' ) !== 'off' && $this->getSetting( 'scrollto' ) )
				){
	...
	function prefix_classes(){
		//megamaxp( $this->item_classes );
		//if( $this->type == 'tabs' ) megamaxp( $this->item_classes );
		$k = 0;
		$found = false;
		foreach( $this->item_classes as $i => $class ){
			//if( $class == 'menu-item' ) $classes[$i] = 'megamaxmenu-item';
			...
			if( $class ){
				if( substr( $class , 0 , 4 ) == 'menu' ){
					$this->item_classes[$i] = 'megamax'.$class;
				}
				else $this->item_classes[$i] = 'megamaxmenu-'.$class;
	...
	function add_class_item_display(){
		...
		if( $this->depth > 0 ){
			...
			$this->item_classes[] = 'megamaxmenu-item-'.$item_display;
			...
			if( $item_display == 'auto' ){
				...
				if( $item_display == 'auto' ){
					...
					switch( $in_sub ){
						...
						case 'tabs-group':
						case 'toggles-group':
							//Ignore, use 'megamaxmenu-toggle' instead
							$item_display = '';
							break;
						case 'tab-content-panel':
						case 'toggles-content-panel':
	...
	function add_class_level(){
		$this->item_classes[] = 'megamaxmenu-item-level-'.$this->getVirtualDepth(); //$this->depth;
	}
	...
	function add_class_layout_columns(){
		...
		$this->item_classes[] = 'megamaxmenu-column megamaxmenu-column-' . $cols;


		//New Row
		if( $this->getSetting( 'clear_row' ) == 'on' ){
			$this->item_classes[] = 'megamaxmenu-clear-row';
		}
	...
	function add_class_submenu(){
		...
		if( $submenu_type ){
			if( in_array( $submenu_type , array( 'mega' , 'flyout' ) ) ){
				$this->item_classes[] = 'megamaxmenu-has-submenu-drop';

				//Show current
				if( ( $this->getSetting( 'show_current' ) == 'on' ) &&
					( in_array( 'megamaxmenu-current-menu-ancestor' , $this->item_classes ) ||
					  in_array( 'megamaxmenu-current-menu-item' , $this->item_classes )
					) ){
					$this->item_classes[] = 'megamaxmenu-active';
				}
	...
	function add_class_disable_padding(){
		...
		if( $disable_padding == 'on' ){
			$this->item_classes[] = 'megamaxmenu-disable-padding';
		}
	}
	...
	function setup_trigger(){
		...
		if( $trigger && $trigger != 'auto' ){
			$this->item_atts['data-megamaxmenu-trigger'] = $trigger;
		}
	}
	...
	function anchor_atts(){
		$atts = array();
		$atts['class']	= 'megamaxmenu-target';	//add MegaMaxMenu specific meta
	...
	function get_anchor( $atts ){
		...
		if( $this->item->title == '--divide--' ){
			return '<div class="megamaxmenu-divider"><hr/></div>';
		}
		...
		if( $this->getSetting( 'highlight' ) == 'on' ){
			$atts['class'].= ' megamaxmenu-highlight';
		}


		//Image
		$image = $this->get_image();
		if( $image ) $atts['class'] .= ' megamaxmenu-target-with-image';


		//Icon
		$icon = $this->getSetting( 'icon' );
		if( $icon ) $icon = megamaxmenu_fa5_convert( $icon , true );
		$icon_classes = apply_filters( 'megamaxmenu_icon_custom_class' , $icon , $this->ID , isset( $this->settings['icon_custom_class'] ) ? $this->settings['icon_custom_class'] : '' );
		if( $icon_classes ){
			$atts['class'] .= ' megamaxmenu-target-with-icon';
			...
			else if( megamaxmenu_op( 'aria_hidden_icons' , 'general' ) == 'on' ){	//TODO deprecate
				$aria = 'aria-hidden="true"';
			}

			$icon = '<'.$icon_tag.' class="megamaxmenu-icon '.$icon_classes.'" '.$aria.$icon_title.'></'.$icon_tag.'>';
		}
		...
		$atts['class'].= ' megamaxmenu-item-layout-'.$layout;
...
//if( $this->id == 604 ) echo $this->item->title;
		...
		if( $content_align != 'default' ){
			$atts['class'].= ' megamaxmenu-content-align-'.$content_align;
		}
//$a.= $this->getSetting( 'submenu_item_content_alignment' );
		...
		if( $layout == 'default' ){
			...
			//If there's an icon, use the default icon layout
				...
				if( function_exists( 'megamaxmenu_icon_layout_default' ) ){
					$layout = megamaxmenu_icon_layout_default( $this );
				}
		...
		$layout_order = megamaxmenu_get_item_layouts( $layout );
		if( !$layout_order ){
			megamaxmenu_admin_notice( __( 'Unknown layout order:', 'megamaxmenu' ).' '.$layout.' ['.$this->item->title.'] ('.$this->ID.')' );
		}


		//No wrap
		if( $this->getSetting( 'no_wrap' ) == 'on' ){
			$atts['class'].= ' megamaxmenu-target-nowrap';
		}
		...
		if( $this->getSetting( 'disable_submenu_indicator' ) == 'on' ){
			...
			$atts['class'].= ' megamaxmenu-noindicator';
		}
		...
		if( $scrollTo ){
			$atts['data-megamaxmenu-scrolltarget'] = $scrollTo;
		}
		...
		if( $this->getSetting( 'disable_text' ) == 'off' ){
			...
			$title .= '<span class="megamaxmenu-target-title megamaxmenu-target-text">';
			...
			$atts['class'].= ' megamaxmenu-item-notext';
		}
		...
		if( $this->item->description ){
			...
			if( ( ( $this->depth == 0 ) && ( $this->get_menu_op( 'descriptions_top_level' ) == 'on' ) ) ||
				...
				if( $title && $divider ) $description.= '<span class="megamaxmenu-target-divider">'.$divider.'</span>';

				$description.= '<span class="megamaxmenu-target-description megamaxmenu-target-text">';
				$description.= $_desc;
				$description.= '</span>';
			}
		}
		...
		if( $this->has_children && megamaxmenu_op( 'aria_controls' , 'general' ) == 'on' ){
			$atts['aria-controls'] = $this->get_submenu_id();
		}


		//Filter attributes
		$atts = apply_filters( 'megamaxmenu_anchor_attributes' , $atts , $this->item->type , $this->ID /* Menu Item ID */ , $this->item->object /* object type */ , $this->item->object_id /* Post ID */ );
		...
		//Get custom pieces
		$custom_pieces = array();
		extract( apply_filters( 'megamaxmenu_custom_item_layout_data' , $custom_pieces , $layout , $this->ID , $this->item->object_id ) );
		...
		if( $display_submenu_indicators && !$disable_submenu_indicator && $submenu_type && in_array( $submenu_type , array( 'mega' , 'flyout' , 'tab-content-panel' ) ) ){
			...
			$a.= "<$indicator_tag class='megamaxmenu-sub-indicator fas fa-angle-down'></$indicator_tag>";
		}

		//Display mobile close toggle - do in JS?
		// if( in_array( $submenu_type , array( 'mega' , 'flyout' , 'tab-content-panel' ) ) ){
		// 	$a.= '<span class="megamaxmenu-sub-indicator-close"><i class="fas fa-times"></i></span>';
		// }
	...
	function get_image(){
		...
		//Ignore mobile?
		if( ( $this->get_menu_op( 'disable_images_mobile' ) == 'on' ) && megamaxmenu_is_mobile( 'disable_images_mobile' ) ){
			return '';
		}

		//Image
		$img = apply_filters( 'megamaxmenu_item_image' , '' , $this );
		if( $img ) return $img;

		//Allow ID filtering
		$img_id = apply_filters( 'megamaxmenu_item_image_id' , $this->getSetting( 'item_image' ) , $this );

		//Allow src filtering
		$img_src = apply_filters( 'megamaxmenu_item_image_src' , '' , $this );
		...
		if( $img_id || $img_src ){
			...
			if( $this->depth > 0 && $this->get_menu_op( 'lazy_load_images' ) == 'on' ){
				$atts['class'].= ' megamaxmenu-image-lazyload';
			...
			$atts = apply_filters( 'megamaxmenu_item_image_attributes' , $atts , $this );
			$attributes = '';
	...
	function get_custom_content(){
		...
		if( $custom_content ){
			...
			if( megamaxmenu_op( 'disable_custom_content' , 'general' ) === 'on' ){
				return "<!-- Custom Content Areas currently disabled $this->ID -->";
			}

			//Pad the custom content wrapper?
			$pad_custom_content = $this->getSetting( 'pad_custom_content' ) == 'on' ? ' megamaxmenu-custom-content-padded' : '' ;
			...
			$html.= '<div class="megamaxmenu-content-block megamaxmenu-custom-content'.$pad_custom_content.$custom_class.'">';
	...
	function get_widget_area(){
		...
		if( $this->getSetting( 'auto_widget_area' ) ){
			...
			if( megamaxmenu_op( 'disable_widget_areas' , 'general' ) === 'on' ){
			...
			if( is_active_sidebar( $custom_area_id ) ){
				...
				$notice = __( 'The widget area is empty.' , 'megamaxmenu' );
				$notice.= ' <a target="_blank" href="'.admin_url( 'widgets.php' ).'">'.__( 'Assign a widget' , 'megamaxmenu' ).'</a>';
				...
				$html.= megamaxmenu_admin_notice( $notice , false );
				return $html;
			}
		}

		//If this is a top level widget and that setting is not enabled, show an admin message
		if( $this->depth == 0 && $widget_area_id && megamaxmenu_op( 'allow_top_level_widgets' , 'general' ) != 'on' ){

			$msg = '<strong>[Menu Item: '. $this->item->title . ']</strong> '. __( 'You have assigned a widget area to a top level menu item.  If you want the widget to appear in a submenu, please attach it to a child menu item.  If you want the widget to appear in the menu bar (always visible), please enable the setting in the MegaMaxMenu Control Panel > General Settings > Widgets > Allow Top Level Widgets', 'megamaxmenu' );
			megamaxmenu_admin_notice( $msg , true );	//Deliberately printed BEFORE the menu rather than within it because the message is so long.
			//$html.= megamaxmenu_admin_notice( $msg , true );
			return $html;
		}
		...
		if( $widget_area_id && is_active_sidebar( $widget_area_id ) ){
			...
			//Evenly divided
			$cols = 'megamaxmenu-column-1-'.$num_widgets;
			if( $num_widgets == 1 ){
				$cols = 'megamaxmenu-column-full';
			}
			...
			if( is_numeric( $widget_area_columns ) ){
				...
				else $cols = 'megamaxmenu-column-1-'.$widget_area_columns;
			}
			...
			$html.= '<ul class="megamaxmenu-content-block megamaxmenu-widget-area megamaxmenu-autoclear">'; //megamaxmenu-row
			$html.= $widget_area;
			$html.= '</ul>';
		}
		//No widgets
			...
			if( $this->type == 'widget_area' ){
				$notice = __( 'Please enter a name for your Custom Widget Area, or assign a Reusable Widget Area to this menu item.' , 'megamaxmenu' );
...
/* A Horizontal Rule */
class MegaMaxMenuItemDivider extends MegaMaxMenuItem{
	function get_start_el(){
		return '<li class="megamaxmenu-divider"><hr/>';
	}
}
