...
*/
		...
		$this->submenu_classes[] = 'megamaxmenu-tabs-group';
		...
		$this->submenu_classes[] = 'megamaxmenu-column';
		$this->submenu_classes[] = 'megamaxmenu-column-'.$tabs_group_layout;
	...
	function alter( &$children ){
		...
		if( !isset( $children[$reference_index] ) ){
			$notice = __( 'To create tabs, add child items to the Tab Item.' , 'megamaxmenu' );
			$this->manual_content = megamaxmenu_admin_notice( $notice , false );;
			return;
		}
...
//echo '>> ' . $this->item->title . ' : ' . implode(',' , $this->item_classes ) .'<br/>';
		...
		if( $this->getSetting( 'show_default_panel' ) == 'on' ){
			$this->item_classes[] = 'megamaxmenu-tabs-show-default';
		}

		//Show Current
		if( $this->getSetting( 'show_current_panel' ) == 'on' ){
			$this->item_classes[] = 'megamaxmenu-tabs-show-current';
		}


		//Dynamic Sizing & Animation
		if( $this->getSetting( 'dynamic_panel_sizing' ) == 'on' ){
			$this->item_classes[] = 'megamaxmenu-tabs-dynamic-sizing';
			if( $this->getSetting( 'dynamic_panel_animation' ) == 'on' ){
				$this->item_classes[] = 'megamaxmenu-tabs-dynamic-sizing-animate';
			}
		}
		...
		//$html.= '<li class="megamaxmenu-tabs">';
	...
	function setup_tab( $umitem ){
			...
			switch( $tab_layout ){
				...
				case 'right':
					...
					if( $tabs_group_layout == 'auto' ){
						...
						$panels_group_layout = megamaxmenu_get_column_complement( $tabs_group_layout );
					}
					break;
		...
		$umitem->submenu_classes[] = "megamaxmenu-tab-content-panel";
		$umitem->submenu_classes[] = "megamaxmenu-column";
		$umitem->submenu_classes[] = "megamaxmenu-column-".$panels_group_layout;

		//Padding
		// if( $this->getSetting( 'panels_padded' ) == 'on' ){
		// 	$umitem->submenu_classes[] = 'megamaxmenu-submenu-padded';
		// }

		//Grid Panel
		if( $this->getSetting( 'panels_grid' ) == 'on' ){
			$umitem->submenu_classes[] = 'megamaxmenu-submenu-grid';
		}
	}
...
class MegaMaxMenuItemTab extends MegaMaxMenuItemDefault{
...
*/
	...
	function get_start_el(){
		...
		if( !( $this->getSetting( 'disable_submenu_on_mobile' ) == 'on' &&
				megamaxmenu_is_mobile( 'disable_submenu_on_mobile' ) ) &&
			$this->has_children ){
			$this->item_classes[] = 'megamaxmenu-has-submenu-drop';
		}
