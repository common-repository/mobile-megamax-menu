 ...
 */
class MegaMaxMenuItemTabs extends MegaMaxMenuItem{
...
//up( $children , 2 );
		...
		$tab_layout_class = 'megamaxmenu-tab-layout-'.$tab_layout;
		...
		$tabs_column_class = 'megamaxmenu-column megamaxmenu-column-'.$tabs_group_layout; //' megamaxmenu-column-1-3';
		$panels_column_class = 'megamaxmenu-column megamaxmenu-column-'.$panels_group_layout; //megamaxmenu-column-2-3';
		
		
		$tabs_default_panel_class = $panels_default_panel_class = '';
		if( $this->getSetting( 'show_default_panel' ) == 'on' ){
			$tabs_default_panel_class = $panels_default_panel_class = 'megamaxmenu-toggle-show-default';
		}
		...
		$tab_group = new MegaMaxMenu_dummy_item( 
						$tabs_id , 
						'toggle_group' , 
						'Toggle Group' , 
						$this->ID ,
		...
		//Panels Group
		$this->settings['menu_item_parent'] = $this->source_id; //$this->ID;
		$panel_group = new MegaMaxMenu_dummy_item( 
						$panels_group_id , 
		...
		//Find Tabs - only menu items, not custom MegaMaxMenu types
...
class MegaMaxMenuItemToggle extends MegaMaxMenuItem{
...
//echo '>> ' . $this->item->title . ' : ' . implode(',' , $this->item_classes ) .'<br/>';
		...
		$this->item_classes[] = 'megamaxmenu-toggle';
		...
		//Data target
		$data_target = ' data-megamaxmenu-toggle-target="#megamaxmenu-panel-'.$this->ID.'"';
		...
		//$atts['data-megamaxmenu-toggle-target'] = '#megamaxmenu-panel-'.$this->ID;
	...
	function get_submenu_wrap_start(){
		...
		$id = 'id="megamaxmenu-panel-'.$this->ID.'"';
		$class = 'class="megamaxmenu-submenu megamaxmenu-toggle-content-panel"';
		$out = "<ul $id $class>";
		return $out;
	}
}
...
class MegaMaxMenuItemToggleGroup extends MegaMaxMenuItem{
	...
	function get_start_el(){
		...
		$this->settings['submenu_type_calc'] = 'toggles-group';
		$classes = array();
		$classes[] = 'megamaxmenu-tabs';
		$classes[] = 'megamaxmenu-toggles-group-wrap';
		$classes = array_merge( $classes , $this->item->classes );

		$class= 'class="'.implode( ' ' , $classes ).'"';

		$id = 'id="megamaxmenu-'.$this->ID.'"';

		return "<li $id $class>";
	}
	function get_submenu_wrap_start(){
		$trigger = $this->getSetting( 'tabs_trigger' );
		$data = 'data-megamaxmenu-toggle-trigger="'.$trigger.'"';
		
		$class = 'class="megamaxmenu-submenu megamaxmenu-toggles-group"';
...
class MegaMaxMenuItemToggleContentPanel extends MegaMaxMenuItem{
...
//echo '<br/><br/>MegaMaxMenuItemToggleContentPanel  ** '. $this->ID;
		...
		$ref_settings = get_post_meta( $this->item->ref_id, MEGAMAXMENU_MENU_ITEM_META_KEY , true );
//up( $ref_settings );
		...
		$classes = array();
		$classes[] = 'megamaxmenu-submenu megamaxmenu-toggle-content-panel';
		$classes[] = 'megamaxmenu-submenu-id-'.$this->item->ref_id;
	...
	function get_end_el(){
		...
		if( isset( $ref_settings['submenu_footer_content'] ) && $ref_settings['submenu_footer_content'] ){ 
			$html.= '<li class="megamaxmenu-submenu-footer megamaxmenu-submenu-footer-id-'.$this->item->ref_id.'">'.$ref_settings['submenu_footer_content'].'</li>';
		}
...
class MegaMaxMenuItemToggleContentPanelsGroup extends MegaMaxMenuItem{
...
*/
	...
	function get_start_el(){
		...
		$classes = array();
		$classes[] = 'megamaxmenu-tabs-panels';
		$classes[] = 'megamaxmenu-toggle-contents-group';
		$classes = array_merge( $classes , $this->item->classes );
