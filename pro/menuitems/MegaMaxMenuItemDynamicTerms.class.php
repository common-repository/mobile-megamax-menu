...
class MegaMaxMenuItemDynamicTerms extends MegaMaxMenuItemDynamic{
...
//echo '<h3>dynamic_terms ' . $this->ID. ' || '. $this->source_id . ' || ' .  $this->item->object_id . '</h3><br/>';
		...
		if( MEGAMAXMENU_ALLOW_TOP_LEVEL_DYNAMIC_ITEMS || $this->depth > 0 ){
			...
			if( empty( $taxonomies ) ){
				$this->notice = '<strong>'.__( 'Please select at least one taxonomy in the Dynamic Terms settings' , 'megamaxmenu' ).'</strong>';
			}


			//Allow filtering
			$term_args = apply_filters( 'megamaxmenu_dynamic_terms_args' , $term_args , $this->ID );
			...
			if( empty( $terms ) && $this->getSetting('dt_ignore_empty_results') !== 'on' ){
				$this->notice = '<strong>'.$this->item->title.' ('.$this->ID.')</strong>: '.__( 'No results found' , 'megamaxmenu' );

				if( empty( $taxonomies ) ){
					$this->notice.= '<br/><strong>'.__( 'Please select at least one taxonomy in the Dynamic Terms settings' , 'megamaxmenu' ).'</strong>';
				}
				else if( count( $taxonomies ) > 1 ){
					$this->notice.= '<br/><strong>'.__( 'Make sure you only select the taxonomies that you want to display terms from' , 'megamaxmenu' ).'</strong>';
				}

				$this->notice.= '<br/><em>'.__( 'Taxonomies' , 'megamaxmenu' ).':</em>';
				$this->notice.= '<pre>';
				$this->notice.= print_r( $taxonomies , true );
				$this->notice.= '</pre>';
				$this->notice.= '<br/><em>'.__( 'Query Arguments' , 'megamaxmenu' ).':</em>';
				$this->notice.= '<pre>';
			...
			if( is_wp_error( $terms ) ){
				// $notice = megamaxmenu_admin_notice( $terms->get_error_message() );
				//Invalid
				return;
			}
			...
			foreach( $terms as $term ){
				...
				$term_item = new MegaMaxMenu_dynamic_term_item(
							$term_item_id,
							$this->item,
							array(
								//'title'	=> '['.$term->name.']',
				...
				if( $autocolumns > 0 ){
					...
					if( $_i == 0 || ( isset( $column_map[$_col] ) && $column_map[$_col] == 0 ) ){
						...
						$term_children[] = new MegaMaxMenu_dummy_item(
							$column_id ,
							'column' ,
							'Auto Column' ,
							$this->ID,
							array( 'columns' => '1-'.$autocolumns ),
							array( 'megamaxmenu-autocolumn' )
						);
					}
	...
	function get_start_el(){
		...
		if( $this->term_count == 0 ){
			...
			if( $empty_results_message ){
				$html.= '<li class="megamaxmenu-item megamaxmenu-item-normal"><span class="megamaxmenu-target megamaxmenu-target-empty-terms">'.
		...
		if( $this->notice ){
			$html.= '<li class="megamaxmenu-item">'.megamaxmenu_admin_notice( $this->notice , false ).'</li>';
		}
		...
		if( $view_all_taxonomy && $view_all_taxonomy != 'none' ){
			...
			if( $term_id ){
				...
				if( !is_wp_error( $term_url ) ){
					...
					$html.= '<li class="megamaxmenu-item megamaxmenu-item-normal megamaxmenu-item-view-all"><a href="'.$term_url.'" class="megamaxmenu-target">'.$view_all_link_text.'</a></li>';
				}
			}
		}

		$html = apply_filters( 'megamaxmenu_dt_after' , $html , $this->ID , $this->term_args );
...
class MegaMaxMenuItemDynamicTerm extends MegaMaxMenuItemDefault{
	...
	function init(){
		...
		if( $this->walker->parent_item() ){
			...
			if( $this->walker->parent_item()->getType() == 'tabs' ){
				...
				$this->item_classes[] = 'megamaxmenu-tab';
				...
				if( $this->depth > 0 && ( !isset( $this->settings['submenu_column_default'] ) || $this->settings['submenu_column_default'] == 'auto' ) ){
					...
					//if( $this->term->name == 'China' ) megamaxp( $this->settings );
				}
		...
		if( $this->item->term_id == $queried_object_id && ( $wp_query->is_category || $wp_query->is_tag || $wp_query->is_tax ) && $queried_object->taxonomy == $this->item->taxonomy_slug ){
			$this->item_classes[] = 'megamaxmenu-current-menu-item';	//hasn't been prefixed yet
		}
	}
	...
	function get_anchor( $atts ){
		...
		$this->settings['item_image'] = apply_filters( 'megamaxmenu_dt_image' , '' , $this->ID , $term );
		$image = $this->get_image();
		...
		if( $icon ){
			...
			$icon_tag = $this->get_menu_op( 'icon_tag' );
			if( !$icon_tag ) $icon_tag = 'i';
			$icon = '<'.$icon_tag.' class="megamaxmenu-icon '.$icon.'"></'.$icon_tag.'>';
		}
		...
		if( $this->getSetting( 'disable_text' ) == 'off' ){
			...
			if( $this->getSetting( 'dt_display_term_counts' ) == 'on' ){
				$title .= ' <span class="megamaxmenu-term-count">'. MEGAMAXMENU_TERM_COUNT_WRAP_START .$term->count. MEGAMAXMENU_TERM_COUNT_WRAP_END.'</span>';
			}
		...
		//Dynamic Subcontent
		$description = apply_filters( 'megamaxmenu_dt_subcontent' , $description , $term , $this->ID );
		...
		$atts = apply_filters( 'megamaxmenu_anchor_attributes' , $atts , 'megamaxmenu-dynamic-term' , $this->source_id /* Menu Item ID */ , 'term' /* object type */ , $term->term_id /* Post ID */ );
		...
		//Get custom pieces
		$custom_pieces = array();
		extract( apply_filters( 'megamaxmenu_custom_item_layout_data' , $custom_pieces , $layout , $this->ID , $term->term_id ) );
