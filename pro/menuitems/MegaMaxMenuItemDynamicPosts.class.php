...
class MegaMaxMenuItemDynamicPosts extends MegaMaxMenuItemDynamic{
...
//echo '<h3>dynamic_posts ' . $this->ID. '</h3>';
		...
		if( MEGAMAXMENU_ALLOW_TOP_LEVEL_DYNAMIC_ITEMS || $this->depth > 0 ){
			...
			if( $posts === false ){
				...
				//Allow filtering
				$post_args = apply_filters( 'megamaxmenu_dynamic_posts_args' , $post_args , $this->ID );
...
//echo "$post_count / $autocolumns = $items_per_column";
			...
			foreach( $posts as $p ){
				...
				$post_item =
					new MegaMaxMenu_dynamic_post_item(
							$post_item_id,
							$this->item,
							array(
				...
				if( $autocolumns > 0 ){
					...
					if( $_i == 0 || ( isset( $column_map[$_col] ) && $column_map[$_col] == 0 ) ){
						...
						$post_children[] = new MegaMaxMenu_dummy_item(
							$column_id ,
							'column' ,
							'Auto Column' ,
							$this->ID,
	...
	function get_start_el(){
		...
		if( $this->post_count == 0 ){
			...
			if( $empty_results_message ){
				$html.= '<li class="megamaxmenu-item megamaxmenu-item-normal"><span class="megamaxmenu-target megamaxmenu-target-empty-posts">'.
	...
	function get_end_el(){
		...
		//Allow filtering
		$html = apply_filters( 'megamaxmenu_dp_after' , $html , $this->ID , $this->post_args );
...
class MegaMaxMenuItemDynamicPost extends MegaMaxMenuItemDefault{
...
//echo $this->post->post_title . '<br/>';
		...
		if( $this->walker->parent_item() ){
			...
			if( $this->walker->parent_item()->getType() == 'tabs' ){
				...
				//$this->item_atts['data-megamaxmenu-toggle-target'] = '#megamaxmenu-panel-'.$this->ID;
	...
	function get_anchor( $atts ){
		...
		if( $this->getSetting( 'disable_text' ) == 'off' ){
			...
			$title = apply_filters( 'megamaxmenu_dp_title' , $title , $this->source_id , $p->ID );

			$title = '<span class="megamaxmenu-target-title megamaxmenu-target-text">' . $title . '</span>';
		}
		...
		//Filter attributes
		$atts = apply_filters( 'megamaxmenu_anchor_attributes' , $atts , 'megamaxmenu-dynamic-post' , $this->source_id /* Menu Item ID */ , 'post' /* object type */ , $p->ID /* Post ID */ );
		...
		if( $subcontent_type != 'none' ){
			...
			switch( $subcontent_type ){
				...
				case 'custom':
					$description = apply_filters( 'megamaxmenu_dp_subcontent' , $description , $p , $this->ID );
					break;
		...
		//Get custom pieces
		$custom_pieces = array();
		extract( apply_filters( 'megamaxmenu_custom_item_layout_data' , $custom_pieces , $layout , $this->ID , $p->ID ) );
