 ...
 */
class MegaMaxMenuItemMenuSegment extends MegaMaxMenuItem{
	...
	function get_start_el(){
		...
		if( $menu_segment == '_none' || !$menu_segment ){
			$html.= '<li>'.megamaxmenu_admin_notice( 'Please set a segment for <strong>'.$this->item->title .' ('.$this->ID.')</strong>', false ).'</li>';
			return $html.='<!-- no menu set -->';
		}

		$menu_object = wp_get_nav_menu_object( $menu_segment );
		if( !$menu_object ){
			$html.= '<li>'.megamaxmenu_admin_notice( 'No menu with ID '.$menu_segment.' for menu item: <strong>'.$this->item->title .' ('.$this->ID.')</strong>', false ).'</li>';
		...
		if( $this->getSetting( 'segment_transient_cache' ) == 'on' ){
			$transient_key = 'megamaxtk_mseg_'.$this->ID; //$this->get_transient_key( 'mseg_' );
		...
		if ( false === ( $segment_html ) ) {
			...
			$menu_segment_args = array( 
				...
				'walker'		=> $this->walker, 
				'depth'			=> 0,
				'megamax_instance'	=> $this->args->megamax_instance,
				'megamax_segment'	=> $this->ID,
			);

			//Autocolumn handling
			$menu_segment_args['megamax_segment_data'] = array(
				'submenu_autocolumns' => $this->getSetting( 'submenu_autocolumns' ),
			);
			add_filter( 'wp_nav_menu_objects' , 'megamaxmenu_menu_segment_objects_filter' , 10 , 2 );

			//Record the settings so we can easily replace when force-filtering
			$menu_segment_args['megamax_segment_args'] = $menu_segment_args;
...
function megamaxmenu_menu_segment_objects_filter( $sorted_menu_items , $args ){
//Need to defer still

	if( isset( $args->megamax_segment_data ) ){
		if( isset( $args->megamax_segment_data['submenu_autocolumns'] ) ){
			$autocolumns = $args->megamax_segment_data['submenu_autocolumns'];

			if( $autocolumns > 0 ){

				$group_id = $args->megamax_segment;		//menu segment ID

				//megamaxp( $args );
				if( $args->megamax_segment == 83 ){
					//megamaxp( $sorted_menu_items , 2 );
					...
					foreach( $column_map as $_col => $column_count ){
						...
						$new_parents[] = new MegaMaxMenu_dummy_item(
									$column_id , 
									'column' , 
									'Auto Column' , 
									$group_id,
						...
						$k = 0;
						//megamaxp( $sorted_menu_items );
						while( $k < $column_count ){
							//megamaxp( $sorted_menu_items[$child_index] );
					...
					remove_filter( 'wp_nav_menu_objects' , 'megamaxmenu_menu_segment_objects_filter' );
				}
			}
		}
	}
...
add_action( 'megamaxmenu_after_menu_item_save' , 'megamaxmenu_clear_menu_segment_transient' , 10 , 1 );
function megamaxmenu_clear_menu_segment_transient( $menu_item_id ){
	delete_transient( 'megamaxtk_mseg_'.$menu_item_id );
}