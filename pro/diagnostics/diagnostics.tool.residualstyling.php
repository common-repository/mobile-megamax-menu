	...
	<div class="umd-tool-body">
		<div class="umd-tool-steps">
			...
			<div class="umd-tool-step">
				<h3>Overview</h3>
				<p>Residual styling is when CSS styles from your theme interfere with MegaMaxMenu's styles.  This is most commonly caused by CSS applied based on an element wrapping MegaMaxMenu.</p>
			...
			<div class="umd-tool-step">
				...
				<div class="umd-tool-rs-menu-chosen">
					...
					<p>If your styles never end up looking correct, the source of the residual styling my be overly generic and not be able to be eliminated by removing a container (try using Chrome Developer Tools to source the issue instead), or the interference may not be from CSS.</p>
				</div>
			...
			<div class="umd-tool-step">
				...
				<a class="umd-tool-action-btn" id="umd-tool-rs-search" href="#" data-nonce="<?php echo wp_create_nonce( 'megamaxmenu_theme_search' ); ?>">Search theme files for menu</a>
				<?php endif; ?>
			...
			<div class="umd-tool-step">
				...
				<div class="umd-tools-rs-manual-integration-skeleton">
					...
					<p>Now that you have the potential locations for the code in the theme, look through those locations in your theme files to locate the correct code block.  If you had an ID hit in your results, that's likely the exact spot.  The point is to replace the theme's entire block of menu code with MegaMaxMenu, effectively moving MegaMaxMenu outside of the theme's menu container.  Remember, it'll look something like (but not exactly like) this:</p>
					...
					<p>We will now write a bit of PHP that says "If MegaMaxMenu is installed, show MegaMaxMenu, otherwise, show the theme menu".  This is the skeleton for that logic.  You will replace the 2 parts that start with "//" with two pieces of code - the part in the IF statement is generated in the Control Panel > Manual Integration by selecting the appropriate theme location and copying and pasting.  The ELSE statement will containt the block of <strong>Theme Menu Code</strong></p>

<p><code><?php echo esc_html( "<?php if( function_exists( 'megamaxmenu' ) ): ?>
	//MEGAMAXMENU MANUAL INTEGRATION CODE GOES HERE - megamaxmenu()
<?php else: ?>
...
<p><code><?php echo esc_html( "<?php if( function_exists( 'megamaxmenu' ) ): ?>
	<!-- This is the MegaMaxMenu Manual Integration code, generated from the Control Panel -->
	<?php megamaxmenu( 'main' , array( 'theme_location' => 'primary' ) ); ?>
<?php else: ?>
...
<?php endif; ?>" ); ?>
				...
				<p>Once you replace the theme menu with MegaMaxMenu, the residual styling should now be resolved, and MegaMaxMenu is manually integrated</p>
